...
static void SAPI_ReceiveDataIndication( uint16 source,
                      uint16 command, uint16 len, uint8 *pData, int8 r_power );
 ...
 * @return      none
 */
void SAPI_ReceiveDataIndication( uint16 source, uint16 command, uint16 len, uint8 *pData,int8 r_power)
{
#if defined ( MT_SAPI_CB_FUNC )
  /* First check if MT has subscribed for this callback. If so , pass it as
  a event to MonitorTest and return control to calling function after that */
  ...
  {
    zb_MTCallbackReceiveDataIndication( source, command, len, pData, r_power);
  }
  else
#endif  //MT_SAPI_CB_FUNC
  {
#if ( SAPI_CB_FUNC )
    zb_ReceiveDataIndication( source, command, len, pData, r_power);
      ...
      switch ( pMsg->event )
      {
        ...
        case AF_INCOMING_MSG_CMD:
          ...
          SAPI_ReceiveDataIndication( pMSGpkt->srcAddr.addr.shortAddr, pMSGpkt->clusterId,
                                    pMSGpkt->cmd.DataLength, pMSGpkt->cmd.Data,pMSGpkt->rssi);
          break;
